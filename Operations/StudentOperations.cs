...
namespace TallyAssignment2.Operations
{
    internal class StudentOperations
    {
        ...
        public void AddStudent()
        {
            ...
            cmd.Parameters.Add("@StudId",System.Data.SqlDbType.Int).Direction = System.Data.ParameterDirection.Output;
            try
            {
                cmd.ExecuteNonQuery();
            }
            ...
            int getId = (int)cmd.Parameters["@StudId"].Value;
            Console.WriteLine("Enter the no of subjects:");
            int no = Convert.ToInt32(Console.ReadLine());
            for (int i = 0; i < no; i++)
            {
                Console.WriteLine("Enter the SubId,name,maxMarks,MarksObtained of Subject {0}",i+1);
                int subId = Convert.ToInt32(Console.ReadLine());
                SqlCommand checkId = new SqlCommand("SELECT COUNT(*) FROM Subject WHERE SubId = '" + subId + "' AND StudId = '" + getId + "'", sqlConnection);
                ...
                while (StudExist == 1)
                {
                    ...
                    checkId = new SqlCommand("SELECT COUNT(*) FROM Subject WHERE SubId = '" + subId + "' AND StudId = '" + getId + "'", sqlConnection);
                ...
                cmd2.CommandText = "uspInsertSubject";
